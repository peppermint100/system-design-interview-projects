...
class ShortenUrlIdGenerator(
    @Value("\${server.id}") private val instanceId: Int,
    private val redisTemplate: RedisTemplate<String, String>
) {
    companion object {
        private const val SEQUENCE_KEY = "url:sequence"
    ...
    fun nextId(): String {
        ...
        val sequence = (redisTemplate.opsForValue()
            .increment(SEQUENCE_KEY) ?: 0) % MAX_SEQUENCE

        return buildString {
            append(System.currentTimeMillis())
            append(instanceId.toString().padStart(2, '0'))
